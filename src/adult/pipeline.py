...
def _fill_na(df: pd.DataFrame, cols: str | list[str], fill_value: typing.Any):
    if cols:
        df.loc[:, cols] = df[cols].fillna(fill_value)
    else:
...
class NAFiller(ColumnTransformer):
    def __init__(self, columns: str | list[str], fill_value: typing.Any):
        mapping = FunctionTransformer(partial(_fill_na, columns=columns, fill_value=fill_value))
        super().__init__([("fill_na", mapping, columns)], remainder="keep")


class ColumnMap(ColumnTransformer):
    def __init__(self, column: str, mapping: dict):
        mapping = FunctionTransformer(lambda s: s.map(mapping))
        super().__init__([(column, mapping, column)], remainder="keep")
...
class NativeCountryMapping(ColumnTransformer):
    ...
    def __init__(self):
        ...
        super().__init__([("native_country", _transform, "native_country")], remainder="keep")
...
class WorkClassMapping(ColumnMap):
    ...
    def __init__(self):
        super().__init__(AdultData.Column.WORK_CLASS,self.WORKCLASS_MAPPING)
