...
from statlog.data import COLS_FEATURE_CATEGORICAL
from statlog.features import feature_registry, FeatureName


class ModelFactory:

    DEFAULT_FEATURES = (FeatureName.NUMERICAL, FeatureName.CATEGORICAL)

    @classmethod
    def create_lightgbm(cls):
        fc = FeatureCollector(*cls.DEFAULT_FEATURES, registry=feature_registry)
        return (
            LightGBMVectorClassificationModel(categorical_feature_names=COLS_FEATURE_CATEGORICAL)
