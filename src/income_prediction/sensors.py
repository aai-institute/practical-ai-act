...
def model_version_trigger(context, mlflow_session: MlflowSession):
    ...
    if context.cursor != ver:
        model_container_request = dg.RunRequest(run_key=f"model-container-build-{model_name}-{ver}",
                                run_config={"resources": {"model_version": {
                                    "config": ModelVersion(version=ver,
                                                           uri=uri).model_dump(), }}, },
                                tags={"model_version": ver, "model_uri": uri},
                                job_name=model_container_job.name)
        nannyml_container_request = dg.RunRequest(run_key=f"model-nannyml-build-{model_name}-{ver}",
                                run_config={"resources": {"model_version": {
                                    "config": ModelVersion(version=ver,
                                                           uri=uri).model_dump(), }}, },
                                tags={"model_version": ver, "model_uri": uri},
