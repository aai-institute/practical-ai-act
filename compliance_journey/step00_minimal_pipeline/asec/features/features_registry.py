...
from asec.features.base import Feature, TakeColumn, MappedColumn, ColumnDifference


class FeatureFactoryRegistry:
    _factory_registry = {}

    @classmethod
    def register_factory(
        cls, identifier: Hashable, feature_factory: Callable[[], Feature | Pipeline]
    ):
        cls._factory_registry[identifier] = feature_factory

    @classmethod
    def get_registered_keys(cls) -> list[Hashable]:
        return list(cls._factory_registry.keys())

    @classmethod
    def get_registered_factory(
        cls, identifier: Hashable
    ) -> Callable[[], Feature | Pipeline]:
        return cls._factory_registry[identifier]
    ...
    def collect_features(
        ...
        feature_list = []
        for identifier, factory in cls._factory_registry.items():
...
FeatureFactoryRegistry.register_factory(
    AdultData.Column.SEX,
...
FeatureFactoryRegistry.register_factory(
    AdultData.Column.MARITAL_STATUS,
    lambda: make_pipeline(
...
FeatureFactoryRegistry.register_factory(
    ...
    lambda: make_pipeline(
        ColumnDifference(
            AdultData.Column.CAPITAL_GAIN, AdultData.Column.CAPITAL_LOSS, NET_CAPITAL
        ),
        StandardScaler(),
    ),
)
...
for col in AdultData.COLS_NUMERIC:
    FeatureFactoryRegistry.register_factory(
        col,
        lambda: make_pipeline(TakeColumn(col.value), StandardScaler()),
    )
